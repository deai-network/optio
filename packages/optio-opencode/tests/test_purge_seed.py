"""purge_seed fully expunges an opencode seed (doc + GridFS blob).

Mirrors optio_claudecode/tests/test_purge_seed.py — the opencode wrapper is
a thin suffix-binding re-export of the same optio_agents seed engine.
"""
...
from optio_opencode import purge_seed
...
async def mongo_db():
    ...
    db_name = f"optio_oc_purge_seed_{os.getpid()}"
    db = client[db_name]
    yield db
