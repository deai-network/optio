"""Conversation-mode file-download tests (Stage 7 Task 7.2).

File-disjoint units that don't need a live ``agy``:
  * the listener's GET /download endpoint (real HTTP, fake download_reader);
  * AntigravityTaskConfig.file_download validation;
  * compose_agents_md injecting the optio-file: downloadables block.

The workdir-confinement guard itself lives in the session's _read_download (agy
deliverables land under <workdir>/home/.gemini/antigravity/artifacts/, already
inside the workdir); the listener maps its ValueError("forbidden")/("too-large")
to 403/413. Mirrors optio-grok's test_file_download (grok ← agy renames).
"""
