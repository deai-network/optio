"""ConversationListener unit tests against a fake Antigravity Conversation.

The listener is the engine-neutral SSE/route surface the dashboard consumes; it
is structurally identical to grok's (adapted from optio-claudecode). It only
observes and forwards — attaching/detaching viewers never influences task state.

Antigravity's real conversation runs every turn ``--dangerously-skip-permissions``
(design §7), so the /permission route is a no-op seam in practice; it is still
exercised here with a fake that fires, to prove the shared route works.
"""
...
async def test_replay_to_late_subscriber(listener):
    ...
    conv.fire({"type": "user", "conversationId": "c1", "text": "hi"})
    conv.fire({"type": "assistant", "conversationId": "c1", "text": "there"})
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{url}/events", headers=_auth("pw")) as resp:
            ...
            assert replay[1]["type"] == "assistant"
...
async def test_control_route_forwards_to_conversation(listener):
    conv, lst, url = listener
    async with aiohttp.ClientSession() as s:
        r = await s.post(f"{url}/control", json={"id": "model", "value": "gemini-2.5-pro"}, headers=_auth("pw"))
        assert r.status == 200 and conv.controls == [("model", "gemini-2.5-pro")]
...
async def test_stop_returns_promptly_with_open_sse(listener):
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{url}/events", headers=_auth("pw")) as resp:
            conv.fire({"type": "assistant", "conversationId": "c1", "text": "x"})
