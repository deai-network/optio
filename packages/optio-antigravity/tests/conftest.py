"""Shared pytest fixtures for optio-antigravity integration tests.

Fixtures:

* ``shim_install_dir`` — a tmp_path subdir containing symlinks named
  ``agy`` and ``ttyd`` pointing at the package-shipped shim scripts.
  Pass this as both ``agy_install_dir`` and ``ttyd_install_dir`` in
  ``AntigravityTaskConfig`` to bypass real binary detection.
...
Adapted from optio-grok's conftest (grok ← agy renames).
"""
...
def task_root(monkeypatch):
    """Point ``OPTIO_ANTIGRAVITY_TASK_ROOT`` at a SHORT temp dir.

    Session-flow tests run agy inside a detached tmux session; a short
    ``/tmp/agtr-*`` root keeps paths well within limits, mirroring the short
    production taskdir.
    """
    path = tempfile.mkdtemp(prefix="agtr-")
    monkeypatch.setenv("OPTIO_ANTIGRAVITY_TASK_ROOT", path)
    try:
        yield path
    finally:
...
async def mongo_db():
    ...
    db_name = f"optio_antigravity_test_{os.getpid()}"
    db = client[db_name]
    yield db
