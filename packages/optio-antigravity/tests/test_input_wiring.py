...
the input listener reaches agy's tmux TUI via the shared inject helpers."""
import asyncio
...
from optio_antigravity.session import create_antigravity_task
...
@pytest.mark.asyncio
async def test_posted_text_reaches_agy_tmux_inject():
    host = _Host()
    lock = asyncio.Lock()

    async def _human_input(text):
        await host_actions.send_text_to_agy(host, "tmux", "/sock", "optio", text)
...
@pytest.mark.asyncio
async def test_posted_nav_key_reaches_agy_send_keys():
    host = _Host()
    lock = asyncio.Lock()

    async def _human_key(key):
        await host_actions.send_key_to_agy(host, "tmux", "/sock", "optio", key)
