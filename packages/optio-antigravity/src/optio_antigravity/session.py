...
async def run_antigravity_session(
    ...
    # (typed messages + NAV keystrokes) into the agy tmux TUI. One lock serializes
    ...
    async def _agy_body(host: Host, hook_ctx: HookContext) -> None:
        ...
        nonlocal cred_watch_task, auth_scrape_task, input_runner
        ...
        async def _human_key(key: str) -> None:
            await host_actions.send_key_to_agy(
        ...
        if (
            ...
            try:
                if not await cred_watcher.capture_gate_ok(host):
                    ...
                    # capture-time stamp: persist metadata.accounts (agy has a
                    # single Google account → a 1-element list; EMPTY → []), then
                    # hand on_seed_saved the first account's human-readable summary
                    # (2nd arg; None when there is no analyzed account). Mirrors
                    # optio-claudecode.
...
async def _maybe_refresh_on_resume(
    host: Host, hook_ctx: HookContext, config: AntigravityTaskConfig, protocol,
) -> list[str]:
...
def create_antigravity_task(
    ...
    # drive agy's TUI menus, reached via the control upstream. Conversation mode
    # (Stage 6) carries the live chat widget only when conversation_ui is on;
    # otherwise no widget.
    if config.conversation_ui:
