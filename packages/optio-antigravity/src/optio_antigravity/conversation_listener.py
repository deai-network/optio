"""Per-task conversation listener — the opt-in dashboard gate for optio-antigravity.

Exposes one running AntigravityConversation over HTTP, reached through the
optio-api widget proxy (which injects the basic-auth credential):
  ...
  POST /control    — {id, value}            -> conversation.set_control
                     (agy has no inline switch — the model control restarts the
                     session with the next turn's --model; Stage 7)
...
Structurally identical to optio-grok's / optio-claudecode's ConversationListener
— the listener is engine-neutral. Antigravity's own turns run
``--dangerously-skip-permissions`` (design §7), so ``on_permission_request`` is a
no-op seam there and the /permission route never fires in practice; it is kept
for surface parity and correlated (if ever used) by the request's raw ``id``.
...
class ConversationListener:
    ...
    async def _on_permission_request(self, request) -> PermissionDecision:
        ...
        # with its request id. The conversation stores the whole request object
        # as PermissionRequest.raw, so `id` is the correlation key. (Antigravity
        # never reaches this path — its turns are skip-permissions, design §7.)
