"""AGENTS.md composition for optio-antigravity.

The Antigravity CLI (``agy``) reads an ``AGENTS.md`` file in its workdir (it
also honors ``GEMINI.md``). The base prompt teaches ``agy`` how to coordinate
with the host harness — which log file to append status/deliverable/done/error
lines to, where to put deliverable files — and the consumer's own task
description is then appended verbatim.

Mirrors optio-grok's ``compose_agents_md`` (both agents read AGENTS.md) with
two divergences: (1) there is no grok-style identity block — ``agy`` does not
mis-identify its environment; (2) the resume section names ``agy``'s own state
tree (``home/.gemini/antigravity``) as what survives a resume. The resume
section is gated on ``supports_resume``.
"""
...
- **Your `home/.gemini/antigravity/` directory — Antigravity's own state
  store (the conversation transcript, artifacts, settings) — IS preserved
  across resumes**, so your history travels with you even when the
    ...
    *,
    ...
    Parameters:
      ...
      host_protocol: when False, omit the keyword-protocol documentation
        block entirely (a conversation-mode concern; the branch is kept).
        Default True — iframe mode's only completion signal is the optio.log
        keyword channel.
      ...
      supports_resume: when True (default), the resume-awareness section is
        ...
        ``home/.gemini/antigravity`` survives across resumes.
      file_download: when True, append the downloadables instruction block so
        ``agy`` offers files to the human via the ``optio-file:`` sentinel link
    ...
    if host_protocol:
        # ``browser="redirect"``: agy's first-launch Google OAuth may shell out
        # to open a URL; documenting the BROWSER: keyword lets the parser
        # surface it to the operator (mirrors grok).
        documentation = build_log_channel_prompt(ProtocolFeatures(browser="redirect"))
        base_prompt_pre = _INTRO + documentation
    else:
        base_prompt_pre = ""
    if supports_resume:
