"""Shared pytest fixtures for optio-kimicode integration tests.
...
Adapted from optio-grok's conftest (grok+ttyd → kimi renames).
"""
...
def shim_install_dir(tmp_path: pathlib.Path) -> pathlib.Path:
    """Tmp dir containing a symlink to the kimi shim."""
    target = tmp_path / "shims"
    target.mkdir()
    source = TESTS_DIR / "kimi-shim.sh"
    os.chmod(source, 0o755)
    os.symlink(source, target / "kimi")
    return target


@pytest.fixture
def task_root(monkeypatch):
    """Point ``OPTIO_KIMICODE_TASK_ROOT`` at a SHORT temp dir.

    Keeps a session's taskdir out of the operator's real home and well within
    path limits.
    """
    path = tempfile.mkdtemp(prefix="kktr-")
    monkeypatch.setenv("OPTIO_KIMICODE_TASK_ROOT", path)
    try:
        yield path
    finally:
...
async def mongo_db():
    ...
    db_name = f"optio_kimicode_test_{os.getpid()}"
    db = client[db_name]
    yield db
