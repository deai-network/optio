"""Per-task conversation listener — the opt-in dashboard gate for optio-kimicode.

Exposes one running KimiCodeConversation over HTTP, reached through the optio-api
widget proxy (which injects the basic-auth credential):
  ...
  POST /control    — {id, value}            -> conversation.set_control
                     (id="model" -> INLINE session/set_model; thinking/mode ->
                     session/set_config_option — no process restart)
...
correlated by the ACP JSON-RPC ``id`` of the ``session/request_permission``
request — KimiCodeConversation hands the whole JSON-RPC object to the handler
...
class ConversationListener:
    ...
    async def _on_permission_request(self, request) -> PermissionDecision:
        ...
        # with its JSON-RPC id. KimiCodeConversation stores the whole ACP request
    ...
    async def _handle_control(self, request: web.Request) -> web.Response:
        # Generic session-control round-trip: {id, value} -> set_control. The
        # model is just id="model" (INLINE session/set_model); thinking/mode go
        # through session/set_config_option. Replaces the bespoke /model route.
        if not self._authorized(request):
            return web.json_response({"ok": False}, status=401)
        try:
            payload = await request.json()
