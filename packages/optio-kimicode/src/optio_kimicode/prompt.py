"""AGENTS.md composition for optio-kimicode.

Kimi Code CLI reads an ``AGENTS.md`` file in its workdir as project memory.
The base prompt teaches kimi how to coordinate with the host harness (which
log file to append status/deliverable/done/error lines to, where to put
deliverable files); the consumer's own task description is then appended
verbatim.

Ported from optio-grok's ``compose_agents_md`` (both agents read AGENTS.md).
The resume-awareness section is gated on ``supports_resume`` and names the
per-task kimi session store (``home/.kimi-code``) as the state that survives
a resume.
"""
...
_KIMICODE_INTRO = """# Coordination protocol with the host harness
...
- **Your `home/.kimi-code/` directory — the kimi session store
  (conversation history, session state) — IS preserved across
  resumes**, so your history travels with you even when the underlying
  process and host change.
    ...
    *,
    ...
    Parameters:
      ...
      file_download: when True, append the downloadables instruction block so
        kimi offers files to the human via the ``optio-file:`` sentinel link
    ...
    if host_protocol:
        ...
        base_prompt_pre = _KIMICODE_INTRO + documentation
    else:
        base_prompt_pre = ""
    if supports_resume:
