...
// The cursor reducer consumes the RAW ACP JSON-RPC objects the listener fans
// out over SSE — the same public protocol grok speaks (session/update
// notifications, the session/request_permission request, the session/prompt
// response as turn-end, plus the synthetic x-optio-* events). The sequences
// below mirror the grok reducer test: both bindings must produce equivalent
// ChatItem[] from the same wire, because they share one ACP reducer.

function play(events: any[], from: ChatState = initialChatState): ChatState {
  return events.reduce((s, ev, i) => reduceCursorEvent(s, ev, i), from);
}
...
describe('cursor ACP event reducer', () => {
  ...
  it('agent_thought_chunk renders as a distinct thinking row, not in the answer', () => {
...
describe('cursor ACP reducer — upload notice → attachment row', () => {
  ...
  it('replays the attachment row from a resumed upload-notice chunk (no optimistic echo — the resume guarantee)', () => {
    const s = play([userChunk('System: upload received, stored in uploads/a.md\n\nlook at it')]);
    const u = s.items.find((i) => i.kind === 'user');
    expect(u && u.kind === 'user' && u.text).toBe('look at it');
    const attach = s.items.find((i) => i.kind === 'activity');
    expect(attach && attach.kind === 'activity' && attach.text).toBe('📎 Attached: a.md');
    expect(s.items.findIndex((i) => i.kind === 'activity')).toBeLessThan(
      s.items.findIndex((i) => i.kind === 'user'));
  });
