// Pure event reducer: raw Grok ACP (Agent Client Protocol, JSON-RPC 2.0)
// messages -> ChatState.
//
// The listener and the widget transport pass the ACP objects through
// untouched; all Grok-specific interpretation lives here (testable without a
// DOM). Wire shapes are pinned in optio-grok's conversation.py:
...
interface GrokChatState extends ChatState {
  turn?: number;
...
// The pending bubble may keep absorbing the in-flight turn only while it is the
// tail (ephemeral tool rows don't count — they are dropped by the next text).
...
// Append a text delta to the in-flight assistant bubble, creating it (with the
// current turn's synthetic msgId) if absent or no longer the tail.
...
export function reduceGrokEvent(state: ChatState, ev: any, seq: number): ChatState {
  return reduce(state as GrokChatState, ev, seq);
}

function reduce(st: GrokChatState, ev: any, seq: number): GrokChatState {
