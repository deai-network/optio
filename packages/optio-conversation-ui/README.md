...
- `reduceClaudecodeEvent`, `reduceOpencodeEvent`, `historyToChatItems`, `initialChatState`, `ChatItem`, `ChatState` — the engine-neutral chat model and per-engine reducers, for embedding outside the default widget.
  ...
  - Review of Fix 13b (2026-09-15), four fixes:
    ...
    - **`x-optio-pending-requeue`** (new synthetic, widget-emitted event, `{ids: string[]}`): `ConversationView`'s "Send now up to" dispatches it (via `queuedIdsAfter(items, upTo)`, chat.ts) right before the `/steer` POST, naming every OTHER currently-queued bubble the CLI is about to cancel and steering.py is about to re-send under a new uuid. `command_lifecycle`'s `'cancelled'`/`'discarded'`/`'refused'` handling ignores an event for any id in `state.pendingRequeue` outright — the owner ruling ("a cancelled that belongs to our own requeue is ignored") applied literally, instead of applying the note and undoing it once `x-optio-requeued` arrives, which live flashed the bubble (Send now link gone) for up to `turn_end_timeout_s`. A replay carries no such local hint (it is not a wire event), so the note-then-requeue path still handles that case unchanged.
