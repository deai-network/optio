"""Per-task conversation listener — the opt-in dashboard gate for optio-cursor.

Exposes one running CursorConversation over HTTP, reached through the
...
Adapted from optio-grok's ConversationListener. Permissions
are correlated by the ACP JSON-RPC ``id`` of the ``session/request_permission``
request — CursorConversation hands the whole JSON-RPC object to the handler
as ``PermissionRequest.raw``.
...
class ConversationListener:
    def __init__(
        ...
        # live events; cursor streams thousands of agent_thought_chunk per turn,
        # which would otherwise evict the replayed history (emitted first) and
        # leave a LATE-reconnecting viewer with no prior conversation. A late
        # /events serves the durable tier first, then the live ring — monotonic
        # seq across both, so Last-Event-ID resumes correctly across the boundary.
    ...
    async def _on_permission_request(self, request) -> PermissionDecision:
        # The raw session/request_permission already reached viewers via
        # _on_event; here we only park until some operator POSTs /permission
        # with its JSON-RPC id. CursorConversation stores the whole ACP request
        # object as PermissionRequest.raw, so `id` is the correlation key.
