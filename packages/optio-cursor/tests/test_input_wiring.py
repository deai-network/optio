...
the input listener reaches cursor's tmux TUI via the shared inject helpers."""
import asyncio
...
from optio_cursor.session import create_cursor_task
...
def test_iframe_task_advertises_iframe_input_widget():
    t = create_cursor_task(
        process_id="p", name="n",
        config=CursorTaskConfig(consumer_instructions="x", mode="iframe"),
    )
...
@pytest.mark.asyncio
async def test_posted_text_reaches_cursor_tmux_inject():
    host = _Host()
    lock = asyncio.Lock()

    async def _human_input(text):
        await host_actions.send_text_to_cursor(host, "tmux", "/sock", "optio", text)
...
@pytest.mark.asyncio
async def test_posted_nav_key_reaches_cursor_send_keys():
    host = _Host()
    lock = asyncio.Lock()

    async def _human_key(key):
        await host_actions.send_key_to_cursor(host, "tmux", "/sock", "optio", key)
