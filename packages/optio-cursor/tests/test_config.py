...
def test_install_dirs_must_be_absolute():
    with pytest.raises(ValueError):
        CursorTaskConfig(consumer_instructions="x", install_dir="rel/path")
    with pytest.raises(ValueError):
...
def test_session_blob_pair_must_be_symmetric():
    ...
    with pytest.raises(ValueError, match="session_blob"):
        CursorTaskConfig(consumer_instructions="x", session_blob_encrypt=ident)
    with pytest.raises(ValueError, match="session_blob"):
        CursorTaskConfig(consumer_instructions="x", session_blob_decrypt=ident)
    # Both set → accepted.
    c = CursorTaskConfig(
        consumer_instructions="x",
