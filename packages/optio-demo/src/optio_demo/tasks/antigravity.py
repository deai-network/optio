"""Demo tasks for optio-antigravity — the seed lifecycle.

Exposes a static **"Setup Antigravity seed"** task plus seed-pinned run tasks
per captured seed (one ttyd-iframe TUI demo, one transcript-driven conversation
demo). The operator launches setup, logs into Antigravity (``agy``) with a
Google account interactively in the ttyd TUI, then stops the task; on teardown
the environment is captured as a seed and the seed-pinned demo tasks appear
(via in-process ``resync``). Authentication comes from the seed, not an
inherited host identity — ``agy`` runs under HOME-isolation
(``HOME=<workdir>/home``), so the host user's ``~/.gemini`` is not inherited;
the seed supplies the captured Google OAuth credentials + settings instead.

Gating mirrors the wrapper's own seed store: the pinned tasks are driven by
``optio_antigravity.list_seeds`` over the real ``{prefix}_antigravity_seeds``
collection. The demo keeps a small ``{prefix}_demo_antigravity_seeds`` sidecar
(written by ``on_seed_saved``) purely to attach a friendly display name to each
seed; seeds without a name fall back to their seed id.

Defaults to local mode; set the ``OPTIO_ANTIGRAVITY_DEMO_SSH_HOST`` environment
variable to run via SSH on a remote host. Relevant env vars (all optional
except ``_HOST``):

- ``OPTIO_ANTIGRAVITY_DEMO_SSH_HOST`` — enables remote mode.
- ``OPTIO_ANTIGRAVITY_DEMO_SSH_USER`` — default: ``$USER`` on the worker.
- ``OPTIO_ANTIGRAVITY_DEMO_SSH_KEY_PATH`` — default: ``~/.ssh/id_ed25519``.
- ``OPTIO_ANTIGRAVITY_DEMO_SSH_PORT`` — default: ``22``.

Hook walkthrough (mirrors the grok/kimicode demos), wired on each seed-pinned
iframe task:

- ``before_execute`` runs ``whoami`` on the host (proves the hook fires inside
  the session pipeline) and ships ``context.txt`` into the workdir via
  ``copy_file`` (bytes source). The LLM is instructed to read that file, so its
  presence is observable end-to-end.
- ``on_deliverable`` exercises the agent feedback channel (rejects a first
  delivery, nudges, accepts the re-delivery).
- ``after_execute`` reads ``./optio.log`` back via ``read_text_from_host`` and
  reports a one-line summary, proving the hook fires before workdir teardown.
"""
...
DEMO_SEED_COLLECTION_SUFFIX = "_demo_antigravity_seeds"

SEED_SETUP_PROMPT = (
    "This is a one-time setup session for a human operator. You do not "
    "need to do anything. The operator will log into Antigravity with a "
    "Google account directly in this terminal — agy opens a browser (or "
    "prints a device-code URL over SSH) to complete the OAuth flow — then "
    "stop this task. Their captured credentials/settings are then stored "
    "as a reusable seed and seed-pinned demo tasks appear automatically. "
    "Do not run setup commands or narrate; if the operator asks you "
    "something, answer briefly, otherwise stay idle."
)


def _resolve_ssh_config() -> SSHConfig | None:
    host = os.environ.get("OPTIO_ANTIGRAVITY_DEMO_SSH_HOST")
    if not host:
        return None
    user = (
        os.environ.get("OPTIO_ANTIGRAVITY_DEMO_SSH_USER")
        or os.environ.get("USER")
        or "root"
    )
    key_path = os.environ.get(
        "OPTIO_ANTIGRAVITY_DEMO_SSH_KEY_PATH",
        os.path.expanduser("~/.ssh/id_ed25519"),
    )
    port_raw = os.environ.get("OPTIO_ANTIGRAVITY_DEMO_SSH_PORT", "22")
    try:
        port = int(port_raw)
    except ValueError:
        raise RuntimeError(
            f"OPTIO_ANTIGRAVITY_DEMO_SSH_PORT must be an integer, got {port_raw!r}"
        )
...
async def _before_execute(hook_ctx: HookContext) -> None:
    ...
    hook_ctx.report_progress(None, f"agy will run as {out.strip()}")
...
_on_deliverable = make_feedback_on_deliverable("antigravity-demo")
...
def _make_on_seed_saved(db, prefix: str, fw):
    ...
    async def _on_seed_saved(seed_id: str, info: str | None = None) -> None:
        ...
        print(f"[antigravity-demo] seed saved {seed_id}: {info}")
...
async def get_tasks(services: dict) -> list[TaskInstance]:
    ...
    tasks: list[TaskInstance] = [
        ...
        create_task(
            ...
            description=(
                ...
                "seed. New seed-pinned demo tasks appear afterward."
            ),
            config=AntigravityTaskConfig(
                ...
                # fs_isolation left at its default (mirrors the grok/kimicode
                # seed-setup task): agy writes its login state under the
    ...
    # One seed-pinned demo task per recorded seed, gated on the real
    # antigravity seed store (mirrors the wrapper). Friendly names come from
    # the sidecar.
        ...
        tasks.append(
            create_task(
                ...
                config=AntigravityTaskConfig(
                    ...
                    # so auto-approve every tool action (agy's
                    # --dangerously-skip-permissions) rather than stalling on a
        ...
        tasks.append(
            create_task(
                ...
                description=(
                    ...
                    "agent in the dashboard, switch the model via session "
                    "controls."
                ),
                config=AntigravityTaskConfig(
                    ...
                    file_download=True,
                    # agy turns run skip-permissions (no interactive gate —
                    # design §7), so the conversation is a pure chat surface.
