"""Demo tasks for optio-grok — the seed lifecycle.

Exposes a static **"Setup Grok seed"** task plus seed-pinned run tasks per
captured seed (one ttyd-iframe demo, one conversation demo). The operator
launches setup, logs into Grok Build interactively in the ttyd TUI, then
stops the task; on teardown the environment is captured as a seed and the
seed-pinned demo tasks appear (via in-process ``resync``). Authentication
comes from the seed, not an inherited host identity — grok runs under
HOME-isolation (``HOME=<workdir>/home``), so the host user's ``~/.grok`` is
not inherited; the seed supplies ``auth.json`` / ``config.toml`` instead.

Gating mirrors the wrapper's own seed store: the pinned tasks are driven by
``optio_grok.list_seeds`` over the real ``{prefix}_grok_seeds`` collection.
The demo keeps a small ``{prefix}_demo_grok_seeds`` sidecar (written by
``on_seed_saved``) purely to attach a friendly display name to each seed;
seeds without a name fall back to their seed id.

Defaults to local mode; set the ``OPTIO_GROK_DEMO_SSH_HOST`` environment
variable to run via SSH on a remote host. Relevant env vars (all optional
except ``_HOST``):

- ``OPTIO_GROK_DEMO_SSH_HOST`` — enables remote mode.
- ``OPTIO_GROK_DEMO_SSH_USER`` — default: ``$USER`` on the worker.
- ``OPTIO_GROK_DEMO_SSH_KEY_PATH`` — default: ``~/.ssh/id_ed25519``.
- ``OPTIO_GROK_DEMO_SSH_PORT`` — default: ``22``.

Hook walkthrough (mirrors the claudecode/opencode demos), wired on each
seed-pinned task:
...
DEMO_SEED_COLLECTION_SUFFIX = "_demo_grok_seeds"

SEED_SETUP_PROMPT = (
    "This is a one-time setup session for a human operator. You do not "
    "need to do anything. The operator will log into Grok Build directly "
    "in this terminal, then stop this task — their credentials/settings "
    "are then captured as a reusable seed and seed-pinned demo tasks "
    "appear automatically. Do not run setup commands or narrate; if the "
    "operator asks you something, answer briefly, otherwise stay idle."
)


def _resolve_ssh_config() -> SSHConfig | None:
    host = os.environ.get("OPTIO_GROK_DEMO_SSH_HOST")
    if not host:
        return None
    user = (
        os.environ.get("OPTIO_GROK_DEMO_SSH_USER")
        or os.environ.get("USER")
        or "root"
    )
    key_path = os.environ.get(
        "OPTIO_GROK_DEMO_SSH_KEY_PATH",
        os.path.expanduser("~/.ssh/id_ed25519"),
    )
    port_raw = os.environ.get("OPTIO_GROK_DEMO_SSH_PORT", "22")
    try:
        port = int(port_raw)
    except ValueError:
        raise RuntimeError(
            f"OPTIO_GROK_DEMO_SSH_PORT must be an integer, got {port_raw!r}"
        )
...
async def _before_execute(hook_ctx: HookContext) -> None:
    ...
    hook_ctx.report_progress(None, f"grok will run as {out.strip()}")
...
_on_deliverable = make_feedback_on_deliverable("grok-demo")
...
def _make_on_seed_saved(db, prefix: str, fw):
    ...
    async def _on_seed_saved(seed_id: str, info: str | None = None) -> None:
        ...
        print(f"[grok-demo] seed saved {seed_id}: {info}")
...
async def get_tasks(services: dict) -> list[TaskInstance]:
    ...
    # One seed-pinned demo task per recorded seed, gated on the real grok
