...
SEED_SETUP_PROMPT = (
    ...
    "login` in this terminal. Cursor tries to open the authentication URL "
    "in a browser; the harness intercepts that and surfaces the URL to the "
    "operator on a `BROWSER:` line, so they can complete the OAuth in their "
    "own browser. (If cursor prints the URL to the terminal instead of "
    "opening it — e.g. over SSH — surface that URL on a `BROWSER:` line "
    "yourself.) Once login succeeds, the operator stops this task: their "
    "credentials/settings are captured as a reusable seed and seed-pinned "
    "demo tasks appear automatically. (Alternative: instead of interactive "
    "login, paste a `CURSOR_API_KEY` into the task config.) Do not run other "
    "setup commands or narrate; if the operator asks you something, answer "
    "briefly, otherwise stay idle."
)
