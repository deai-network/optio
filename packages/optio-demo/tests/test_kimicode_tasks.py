"""Smoke test for the optio-kimicode demo trio.

Mirrors the grok/claudecode demos: an always-present ``kimicode-seed-setup``
task plus seed-pinned run tasks that appear once a kimi seed exists. Gating is
via ``optio_kimicode.list_seeds`` (the real ``{prefix}_kimi_seeds``
collection), so the test drives it by inserting a fake seed doc there.
...
from optio_demo.tasks.kimicode import get_tasks
...
class _FakeFw:
    ...
    exercised by the kimicode factory's ``list_seeds`` gating."""
...
def test_get_tasks_is_async_services_factory():
    """The kimicode demo is a seed-lifecycle factory: async ``get_tasks(services)``."""
    ...
    assert 'process_id="kimicode-seed-setup"' in src
    assert "create_kimicode_task" in src
...
def test_seed_setup_always_and_both_pinned_when_seed_present():
    async def _run():
        ...
        db_name = f"optio_kimicode_demo_test_{os.getpid()}"
        db = client[db_name]
        try:
            ...
            assert "kimicode-seed-setup" in ids
            assert not any(
                pid.startswith("kimicode-demo-seed-")
                or pid.startswith("kimicode-conversation-seed-")
                for pid in ids
            )

            # --- a real kimi seed exists: both pinned tasks appear ----------
            res = await db[f"{PREFIX}{KIMI_SEED_SUFFIX}"].insert_one(
            ...
            assert f"kimicode-demo-seed-{seed_id}" in ids
            assert f"kimicode-conversation-seed-{seed_id}" in ids
        finally:
