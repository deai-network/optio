"""Per-task conversation listener — the opt-in dashboard gate for optio-grok.

Exposes one running GrokConversation over HTTP, reached through the optio-api
widget proxy (which injects the basic-auth credential):
  ...
  POST /control    — {id, value}            -> conversation.set_control
                     (the model control switches INLINE via session/set_model —
                     no process restart; Stage 7)
...
class ConversationListener:
    def __init__(
        ...
        # live events; grok streams thousands of agent_thought_chunk per turn,
    ...
    async def _on_permission_request(self, request) -> PermissionDecision:
        ...
        # with its JSON-RPC id. GrokConversation stores the whole ACP request
