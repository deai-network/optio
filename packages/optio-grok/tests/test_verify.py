"""verify_and_refresh_seed unit tests (fake grok probe, real Mongo).

Host-free verify: plant a seed into a throwaway workdir + GROK_HOME, run one
headless ``grok -p "<probe>"`` challenge-answer via the grok shim (fake_grok's
probe mode), take the verdict from stdout only, and write the rotated auth.json
back into the seed. No real grok binary or network.
"""
...
async def test_alive_and_writes_back_rotated_auth(
    ...
    monkeypatch.setenv("FAKE_GROK_PROBE", "alive")
    ...
    (acct,) = auth.values()
    assert acct["refresh_token"] == "ROTATED-BY-PROBE", auth
...
async def test_dead_on_auth_error(
    ...
    monkeypatch.setenv("FAKE_GROK_PROBE", "dead")
    seed_id = await _make_seed(mongo_db, tmp_path)
...
async def test_prompt_echo_does_not_false_positive(
    ...
    monkeypatch.setenv("FAKE_GROK_PROBE", "echo")
    seed_id = await _make_seed(mongo_db, tmp_path)
...
async def test_exit_code_carries_no_verdict(
    ...
    monkeypatch.setenv("FAKE_GROK_PROBE", "alive_badexit")
    seed_id = await _make_seed(mongo_db, tmp_path)
