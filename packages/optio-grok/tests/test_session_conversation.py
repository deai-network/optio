...
async def test_conversation_ui_file_upload_materialize(
    ...
    optio = await _make_optio(mongo_db, "gkconvupload")
    try:
        task = create_grok_task(
            process_id="gk-conv-files",
        ...
        await optio.launch_and_await_result(
            "gk-conv-files", session_id=None, timeout=60,
        )

        wd = await _wait_widget_data(optio, "gk-conv-files")
        assert wd["showFileUpload"] is True
        assert wd["uploadUrl"] == (
            "{widgetProxyUrl}../../../../widget-upload/"
            f"{mongo_db.name}/gkconvupload/gk-conv-files"
        )
        ...
        rel = await optio.materialize_upload(
            "gk-conv-files", b"hello-bytes", "note.txt",
        )
