...
**Tech Stack:** TypeScript (pnpm workspace, vitest, ts-rest, zod), Python (setuptools, pytest, pydantic), `@clamator/codegen` 0.1.3, `@clamator/protocol`, `@clamator/over-redis`, `clamator-protocol`, `clamator-over-redis`, GNU make, bash.
...
Edit `packages/optio-contracts/package.json` `dependencies` block to add `"@clamator/protocol": "^0.1.3"` (alphabetical placement after `@ts-rest/core`). Keep zod and the others.
...
```json
"@clamator/over-redis": "^0.1.3",
"@clamator/protocol": "^0.1.3",
```
