...
- **Cache population (pragmatic):** grok's headless bootstrap-installer URL is unconfirmed, so Stage 5 seeds the cache from the resolved host `grok` binary (copy/symlink into the cache) when the cache is empty and grok is on PATH; a real vendor-installer download is a documented future refinement (grok also self-updates via `grok update`). The cached binary is the stable path tasks launch — decoupled from the operator's autoupdating `~/.grok`.
