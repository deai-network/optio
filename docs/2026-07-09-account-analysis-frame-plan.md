...
- **Test runner (IMPORTANT — overrides every per-task command below):** there is **NO per-package `.venv`**. Use the **root venv** from the repo root: `.venv/bin/pytest packages/<pkg>/tests/<file>::<test> -q`. The root `.venv` is editable — local `src/` edits are seen immediately, no reinstall. Full suite = `make test` (`pnpm -r test`). Wherever a task step says `cd packages/X && .venv/bin/pytest tests/...`, read it as `.venv/bin/pytest packages/X/tests/...` from repo root. **Excavator** uses the **root** venv `~/deai/excavator/.venv/bin/pytest` (py3.13) — it has all optio packages **editable-linked to `~/deai/optio`**, so frame changes are live there with NO reinstall. Do NOT use `~/deai/excavator/packages/engine/.venv` (a stale py3.11 0.1.3 copy). Run excavator pytest from `~/deai/excavator` (e.g. `.venv/bin/pytest packages/engine/... -q`).
