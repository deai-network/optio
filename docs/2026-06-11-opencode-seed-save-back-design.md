...
3. Provision the workdir, apply `_isolation_env(host)`, and plant `auth.json` + `opencode.json` via a new host-free **`seeds.plant_seed(db, host, *, prefix, …)`** — the extract half of `merge_seed`, db-first (`merge_seed(ctx, …)` requires a `ProcessContext`, which verify doesn't have; `merge_seed` becomes a one-line delegation to `plant_seed`).
