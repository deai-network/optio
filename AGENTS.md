...
`x-optio-queued` / `x-optio-taken` / `x-optio-interrupt` / `x-optio-requeued`.
...
text = send what is queued; `upTo` = send now up to one queued message, a
native-queue agent only) and `POST /interrupt`; stage 1 wires Claude Code
...
Design: `docs/2026-09-13-conversation-steering-design.md`.
